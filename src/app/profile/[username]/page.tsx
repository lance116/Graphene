          ...
          <div className="border border-border mb-4 sm:mb-6">
            ...
            {(() => {
              ...
              for (const evt of data.activity) {
                const key = new Date(evt.date).toLocaleDateString("en-US", { month: "long", year: "numeric" });
              ...
              return Object.entries(grouped).map(([month, events]) => (
                <div key={month}>
                  <div className="px-4 py-2 border-b border-border/50 bg-surface/30">
                    <p className="text-[10px] font-medium text-text">{month}</p>
                  </div>
                  <div className="relative pl-8 pr-4">
                    ...
                    {(() => {
                      ...
                      const groups: { type: string; items: ActivityEvent[] }[] = [];
                      for (const evt of events) {
                        const last = groups[groups.length - 1];
                        if (last && last.type === evt.type) {
                          last.items.push(evt);
                        } else {
                          groups.push({ type: evt.type, items: [evt] });
                        }
                      }
